...
int main ()
{
  ...
  // wierszowa
  #pragma omp parallel for num_threads(3) default(none) reduction(+:suma_parallel) shared(a) schedule(static, 2) private(j) ordered
  for(i=0;i<WYMIAR;i++) {
    int id_w = omp_get_thread_num();
    #pragma omp ordered
    #pragma omp parallel for num_threads(2) default(none) reduction(+:suma_parallel) shared(a) schedule(static, 2) firstprivate(i) firstprivate(id_w) ordered
    for(j=0;j<WYMIAR;j++) {
      suma_parallel += a[i][j];
      #pragma omp ordered
      // dla dekompozycji 1D
