...
//
    ...
    public void zlicz_znaki_przedzial(char znak_start, char znak_koniec) {
        for (int i = 0; i < size_n; i++) {
            for (int j = 0; j < size_m; j++) {
                for (int k = znak_start-33; k < znak_koniec-33; k++) {
                    if (tab[i][j] == tab_symb[k]) histogram_par[k]++;
                }
            }
        }
    }

    public synchronized void drukuj_histogram_przedzial(int id, char znak_start, char znak_koniec) {
        for (int k = znak_start; k < znak_koniec; k++) {
            int count = histogram_par[k - 33];
            System.out.print("Wątek " + id + ": " + (char)k + " ");
            for (int i = 0; i < count; i++) System.out.print("=");
            System.out.println();
        }
    }
